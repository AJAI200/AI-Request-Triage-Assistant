...
class JWTAuthMiddleware(BaseHTTPMiddleware):
    async def dispatch(self, request: Request, call_next):
        ...
        try:
            ...
            process_time_ms = round((time.time() - start_time) * 1000, 2)
            logger.error(f"Unhandled error in auth middleware for {path}: {exc}", exc_info=True)
            res = JSONResponse(status_code=500, content={"detail": "An internal server error occurred. Please try again later."})
            res.headers["X-Process-Time"] = f"{process_time_ms}ms"
            return res
